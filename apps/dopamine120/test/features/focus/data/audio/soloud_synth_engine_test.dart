...
import 'package:dopamine120/features/focus/data/datasources/soloud_synth_engine.dart';
...
void main() {
  ...
  late SoloudSynthEngine engine;

  setUp(() {
    backend = FakeAudioBackend();
    engine = SoloudSynthEngine(
      backend: backend,
      random: Random(1),
      isWeb: false,
    );
  ...
  group('bus', () {
    test('applyProfile pushes the mapped settings', () async {
      ...
      await engine.applyProfile(FocusDimension.jungle.profile);
    ...
    test('temporal distortion bends the active profile', () async {
      await engine.start();
      await engine.applyProfile(FocusDimension.room.profile);
      backend.busSettings.clear();
